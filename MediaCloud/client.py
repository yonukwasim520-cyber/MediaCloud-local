...
class ClientApp:
    ...
    def __init__(self, root):  
        ...
        tk.Label(top, text="Server IP:").pack(side=tk.LEFT, padx=5)  
        tk.Entry(top, textvariable=self.ip, width=30).pack(side=tk.LEFT, padx=5)  
        ...
        # ===== MIDDLE AREA (SCROLLABLE WITH ARROWS) =====  
        middle_frame = tk.Frame(root)
        middle_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)

        # Scroll Buttons Side Control
        scroll_buttons_frame = tk.Frame(middle_frame)
        scroll_buttons_frame.pack(side=tk.RIGHT, fill=tk.Y, padx=5)

        # Up and Down Arrow Buttons
        tk.Button(scroll_buttons_frame, text="▲", font=("Arial", 14), command=self.scroll_up).pack(fill=tk.X, expand=True)
        tk.Button(scroll_buttons_frame, text="▼", font=("Arial", 14), command=self.scroll_down).pack(fill=tk.X, expand=True)
        ...
        self.files_box = tk.Frame(self.canvas)
        self.canvas_frame = self.canvas.create_window((0, 0), window=self.files_box, anchor="nw")

        # Bind resizing and updating scroll region dynamically
        self.files_box.bind("<Configure>", self.on_frame_configure)
        ...
        tk.Button(bottom, text="Upload", command=self.upload_file).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=10)  
        tk.Button(bottom, text="Refresh", command=self.load_files).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=10)  

    # ================= SCROLL ACTIONS (UNLIMITED SPACE) =================
    def scroll_up(self):
        self.canvas.yview_scroll(-9, "units")
    ...
    def update_scroll_limit(self):
        ...
        if bbox:
            x1, y1, x2, y2 = bbox
            self.canvas.configure(scrollregion=(x1, y1, x2, y2 + 5000))
